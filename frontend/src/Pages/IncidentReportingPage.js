...
const MultiCheckbox = ({ label, options, selectedValues, onChange }) => (
  <div style={{ marginBottom: 12 }}>
    ...
    {options.map((opt) => (
      <label
        key={opt}
        style={{ display: "block", fontSize: 14, cursor: "pointer" }}
      >
        <input
          type="checkbox"
...
export default function IncidentReportingPage() {
  ...
  const SYSTEMS = useMemo(
    () => ["Email", "Workstation", "Server", "Website", "Network", "Other"],
    []
  );
  ...
  const validate = () => {
    ...
    if (!form.anonymous && !form.reporterName.trim())
      e.reporterName = "Name is required";
    if (!form.anonymous && !/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(form.email))
  ...
  const handleFile = (e) => {
    ...
    if (tooLarge.length > 0) {
      setErrors((prev) => ({
        ...prev,
        files: "File too large: " + tooLarge.map((f) => f.name).join(", "),
      }));
      return;
    }
      ...
      setTimeout(() => {
        ...
        logoImg.onload = () => {
          try {
            ...
            console.warn(
              "Failed to load logo into PDF, continuing without it."
            );
          }
  ...
  return (
    ...
    >
      ...
      <main style={{ position: "relative", zIndex: 1, padding: "28px 48px" }}>
        {/* Header */}
        <Header user={{ username: "Employee", role: "employee" }} /> <br />
        <div
          style={{
            display: "flex",
            gap: 20,
        ...
        >
          <div>
            ...
            <p className="mt-1 text-base text-gray-400 font-light font-[Poppins]">
              Report suspicious activity, phishing, malware, or data breaches.
            </p>
          </div>
        </div>
        <form
        ...
        >
          ...
          >
            ...
            >
              <h3 style={{ margin: 0, fontSize: 25, fontFamily: "inter" }}>
                Report Details
              </h3>
              <label
                style={{
            ...
            {!form.anonymous && (
              <>
                ...
                />
                {errors.reporterName && (
                  <div style={{ color: "#ff9b9b", marginTop: 6 }}>
                    {errors.reporterName}
                  </div>
                )}
            ...
            >
              Attach files (screenshots, logs, email headers)
            </label>
            <label
              className="glow-input"
              style={{
            ...
            >
              ...
              <input
                ...
                hidden
                onChange={handleFile}
                accept=".png,.jpg,.jpeg,.pdf,.txt,.log,.eml,.msg"
              />
            </label>
            {files.length > 0 && (
          ...
          <aside style={{ display: "flex", flexDirection: "column", gap: 18 }}>
            ...
            >
              <MultiCheckbox
                ...
                options={SYSTEMS}
                selectedValues={form.systemsAffected}
                onChange={(e, v) =>
                  handleMultiCheckbox(e, "systemsAffected", v)
                }
              />
            </div>
            ...
            >
              <MultiCheckbox
                label="Data Affected"
                options={DATA_TYPES}
                selectedValues={form.dataAffected}
                onChange={(e, v) => handleMultiCheckbox(e, "dataAffected", v)}
              />
            </div>
            ...
            >
              <label
                style={{ display: "block", margin: "0 0 6px", fontSize: 16 }}
              >
                Severity
              </label>
              <select
            ...
            >
              <MultiCheckbox
                label="Actions Taken"
                options={ACTIONS}
                selectedValues={form.actionsTaken}
                onChange={(e, v) => handleMultiCheckbox(e, "actionsTaken", v)}
              />
            </div>
            ...
            >
              <div style={{ fontSize: 16, opacity: 0.85, fontFamily: "inter" }}>
                ...
                >
                  Do not include passwords or authentication tokens in the
                  description or attachments. Files are scanned and stored
                  securely.
                </p>
              </div>
            </div>
            ...
            >
              <div style={{ fontSize: 16, fontFamily: "inter" }}>
                ...
                >
                  After submission you will receive an Incident ID to follow up
              ...
              <button
                ...
                onMouseEnter={(e) =>
                  (e.currentTarget.style.boxShadow =
                    "0 0 12px #711bb5, 0 0 25px rgba(113,27,181,0.7)")
                }
